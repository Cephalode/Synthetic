...
export default function LayerCard({ layer, index, onUpdate, onDelete }) {
  ...
  const waveShapes = ['sine', 'square', 'sawtooth', 'triangle', 'noise']
    ...
    <div className="layer-card">
      <div className="layer-header" onClick={() => setCollapsed((c) => !c)}>
        <div className="layer-title">
          ...
          <span className="layer-name">{displayTitle}</span>
          {layer.role && <span className="role-badge">{layer.role}</span>}
      ...
      {!collapsed && (
        <div className="layer-body">
          ...
          <div className="control-group">
            <label>Gain</label>
            <Slider label="Gain" value={layer.gain} min={0} max={1} step={0.01} onChange={set('gain')} unit="" isGain />
          </div>
        </div>
      )}
    </div>
...
function Slider({ label, value, min, max, step, onChange, unit, isPercent, isGain }) {
